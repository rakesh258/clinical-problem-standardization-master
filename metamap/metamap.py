...
class MetaMap:
    ...
    def __init__(self, metamap_exe="metamap", metamaplite_url="http://localhost:8080/",
                 workers=max(multiprocessing.cpu_count() - 1, 1), additional_args='-z -y'):
    ...
    def call_metamap_batch(self, texts, lite=False, batch_size=20):
        ...
        if lite:
            ...
            return self.flatmap(
                self.pool.map(process, [x for x in np.array_split(np.array(texts), batches) if len(x) > 0]))
        else:
    ...
    def call_metamap_raw(self, text):
        ...
        p = subprocess.Popen(
            [self.metamap_exe, "--sldiID", "-Z", "2018AB", "-L", "2018AB", "-N", "--silent", self.additional_args],
